...
When you run `zil serve`, `zil.config` automatically loads `.env` and `.env.local` from both the project root and module directory. **Existing environment variables are never overridden** — real env vars always take precedence over file values.
