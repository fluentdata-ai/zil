...
This returns a fully-configured agent (the type depends on your [framework](/frameworks) — e.g., an ADK `LlmAgent` for the `adk` backend) with:
...
Projects scaffolded with `zil init` include the correct extra in `requirements.txt` automatically. See [Frameworks](/frameworks) for details on each backend.
...
`zil serve` uses this same Session API internally. See [Frameworks](/frameworks#session-api) for event type details across backends.
