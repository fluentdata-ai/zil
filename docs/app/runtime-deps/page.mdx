...
Zil lets you declaratively specify non-Python runtime dependencies in `manifest.yaml`. When you run `zil web --docker` or `zil deploy`, Zil generates ordered Dockerfile `RUN` stanzas that install these dependencies in the correct order — no manual Dockerfile editing required.
...
When you run `zil web --docker`, Zil **regenerates the Dockerfile** from `manifest.yaml` before every `docker build`. This ensures that any changes to `spec.runtime.dependencies` or `spec.tools.host_dependencies` are immediately reflected in the container without manually editing the Dockerfile.
